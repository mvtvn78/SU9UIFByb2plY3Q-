                         ...
                         const char *data, int data_len)
{
    ...
    if (strcmp(topic_buf, topicSub) == 0)
    {
        if (strcmp(data_buf, "1") == 0) {
            printf("LED ON\n");
            ledState =1;
            gpio_set_level(RELAY_PIN, ledState);
            printf("Đèn AC: BẬT\n");
        } 
        else if (strcmp(data_buf, "0") == 0) {
            printf("LED OFF\n");
            ledState =0;
            gpio_set_level(RELAY_PIN, ledState);  // tắt đèn
            printf("Đèn AC: TẮT\n");
        }
    }
}
...
void TouchTask(void*pvParameter)
{
    while(1)
    {
        int val = gpio_get_level(TOUCH_PIN);
        if(val ==1){{
            if(ledState ==0){
                printf("LED ON\n");
                ledState =1;
...
void app_main(void)
{
    ...
    gpio_config_t io_conf_in = {
        ...
        .pull_up_en = GPIO_PULLUP_ENABLE,     // tuỳ chọn
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE        // hoặc IRQ nếu cần
    };
    gpio_config(&io_conf_in);
    xTaskCreate(&TouchTask, "TouchTask", 4096, NULL, 5, NULL);
}